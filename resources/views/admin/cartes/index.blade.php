...
@section('content')
    ...
    <div class="py-4 flex-1 px-6">
        <div class="border-b-2 border-red-100  flex justify-between">
            <h1 class="font-bold pt-4 pb-1 px-2 text-3xl">A-LA-CARTE Items</h1>
            <a href="{{route('admin.cartes.create')}}" class="bg-slate-200 hover:bg-slate-300 font-bold mt-5 py-1 rounded-lg px-4 text-lg">Add A-LA-CARTE Item</a>
        </div>
            ...
            <table id="index" class="display  w-full">
                ...
                <tbody class="text-gray-900 ">
                    ...
                    <tr class=" border-gray-200 border ">
                        ...
                        <td class="p-2">{{$carte->name}}</td>
                        <td class="p-2">{{$carte->description}}</td>
                        <td class="p-2">{{$carte->category->name}}</td>
                        <td class="p-2">{{$carte->price}} &euro;</td>

                        <td class="p-2">
                            <a href="{{route('admin.cartes.edit',$carte->id)}}" class="py-1 text-indigo-700 hover:text-indigo-900 mx-0.5 px-2 rounded-md text-xl shadow-md hover:shadow-lg " title="Edit"><i class="ri-edit-box-line"></i></a>
                            <a onclick="showdelete({{$carte->id}})" class="py-1 text-red-600 hover:text-red-800 mx-0.5 px-2 rounded-md text-xl shadow-md hover:shadow-lg" title="Delete"><i class="ri-delete-bin-4-line"></i></a>
                        </td>
                    </tr>
                    @endforeach
            ...
            <div id="deletemodal"  class="white-card hidden dark:bg-white dark:bg-opacity-10 overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-20 z-50 justify-center items-center md:h-full md:inset-0" x-show="isModal" >
                <div class="relative px-4 w-full max-w-md h-full md:h-auto mx-auto mt-[10%]">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <form class="px-6 pb-4 space-y-6 lg:px-8 sm:pb-6 xl:pb-8" method="POST" action="{{route('admin.cartes.delete')}}">
                        @csrf
